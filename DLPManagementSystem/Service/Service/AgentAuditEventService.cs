            ...
            CancellationToken cancellationToken = default)
        {
            try
            {
                ...
                for (var eventIndex = 0; eventIndex < request.Events.Count; eventIndex++)
                {
                    ...
                    // RE-ENABLED 2026-08-24 (was paused 2026-08-16 alongside the agent-side CLI pause,
                    // now reversed). Independent of the Block-decision alert above, same "independent"
                    // pattern as the integrity-mismatch branch just above. cli.sensitive-command is a
                    // detection-only channel (see ActionKeys.CliSensitiveCommand on the agent side) - the
                    // agent only ever reports this ActionKey when its classifier actually matched
                    // something, so every such event is alert-worthy regardless of what Decision the
                    // agent happened to send (which is never "Block": nothing is actually blocked by this
                    // channel).
                    ...
                    // RE-ENABLED 2026-08-24, same as above - kept for when a health-reporting mechanism
                    // exists again. Currently DEAD CODE IN PRACTICE: CliExecutionHealthChecker/
                    // ReportHealthTransitionAsync were removed agent-side when CLI enforcement moved from
                    // AppLocker to the Explorer DisallowRun/RestrictRun policy (see
                    // CliExecutionPolicyManager.cs's class-level comment) - that mechanism has no
                    // edition/AppIDSvc precondition to report on, so nothing agent-side sends
                    // "CliEnforcementUnavailable" today and this branch will not fire. Left enabled
                    // (rather than re-commented) because it's harmless either way and matches original
                    // intent; do not treat its continued silence as evidence CLI enforcement is healthy.
                    //
                    // Independent of the Block-decision alert above - a health-check transition to
                    // "unavailable" is not a blocked command, it's a report that enforcement itself is
                    // not actually active on this device, so it must alert regardless of Decision (this
                    // agent-side event is never "Block": nothing is being blocked, that's the whole
                    // problem). The agent only ever sends this EventType on a genuine status transition
                    // (not every apply cycle), and uses a different EventType ("CliEnforcementRestored")
                    // once healthy again, so this check does not need to inspect Result/ReasonCode to
                    // avoid alerting on good news.
                        ...
                        && canCreateAlerts)
                    {
                        _db.Alerts.Add(new Alert
                        {
                            ...
                            Description = string.IsNullOrWhiteSpace(envelope.ReasonCode)
                                ? "This device cannot actually enforce CLI execution policy. Any CliExecute=Block policy for this device is not being enforced."
